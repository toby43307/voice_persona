...
def train():
    ...
    # BIT： Troubleshoot: print audio feature frames and estimated duration when testing
    if args.aud_file is not None:
        try:
            aud_path = args.aud_file
            aud_feats = np.load(aud_path)
            aud_frames = aud_feats.shape[0]
            # Estimated duration assuming 25fps hop per feature frame (typical DeepSpeech)
            est_duration_sec = aud_frames * 0.040 # 25 fps = 40 ms per frame
            print(f"[Info] aud_file={aud_path}")
            print(f"[Info] audio feature frames={aud_frames}")
            print(f"[Info] estimated audio duration≈{est_duration_sec:.2f}s (assuming 25 fps)")
        except Exception as e:
            print(f"[Warn] Failed to load aud_file '{args.aud_file}': {e}")
