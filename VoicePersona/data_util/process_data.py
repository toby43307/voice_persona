...
if running_step == 0:
    ...
    #extract_wav_cmd = 'ffmpeg -i ' + vid_file + ' -f wav -ar 16000 ' + wav_file # BIT fix

    # BIT Fix: Force mono and 16kHz to match DeepSpeech expectations
    extract_wav_cmd = f"ffmpeg -y -i \"{vid_file}\" -vn -ac 1 -ar 16000 \"{wav_file}\""

    os.system(extract_wav_cmd)
    extract_ds_cmd = 'python data_util/deepspeech_features/extract_ds_features.py --input=' + id_dir
    os.system(extract_ds_cmd)

    # BIT fix: After feature extraction, report aud.npy length and video stats
    aud_npy_path = os.path.join(id_dir, 'aud.npy')
    if os.path.isfile(aud_npy_path):
        try:
            aud_features = np.load(aud_npy_path)
            aud_len = aud_features.shape[0]
        except Exception as e:
            print(f"Failed to load {aud_npy_path}: {e}")
            aud_len = None
    else:
        print(f"{aud_npy_path} not found")
        aud_len = None
    ...
    print(f"Video fps={fps}, frames={frame_count}, duration={duration_sec:.2f}s")
    if aud_len is not None:
        print(f"aud.npy length (feature frames)={aud_len}")
        print("Note: aud.npy length reflects audio feature frames (window/hop), not video frames.")
    exit()
...
if running_step == 2:
    ...
    fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, flip_input=False) # BIT fix
